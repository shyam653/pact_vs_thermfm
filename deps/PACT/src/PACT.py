...
if "NoPackage" in modelParams:
    ...
    lcf_df = pd.concat([lcf_df, noPackage_layer], ignore_index=True)
...
if "HeatSink" in modelParams:
    ...
    lcf_df = pd.concat([lcf_df, HeatSpreader_layer], ignore_index=True)
    thickness_layers[num_layers +
    ...
    lcf_df = pd.concat([lcf_df, HeatSink_layer], ignore_index=True)
    thickness_layers[num_layers +
...
for ff in flp_files:
    ...
    config_label_df = pd.concat([config_label_df, ff_df[['ConfigFile', 'Label']].drop_duplicates()], ignore_index=True)
