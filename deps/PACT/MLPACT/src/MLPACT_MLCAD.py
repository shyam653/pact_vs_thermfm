...
def parse_args():
    ...
    parser.add_argument("--power-data-csv", default = '../Data/example_power_data.csv')
...
# extracting values:
    # num_sim_steps = 
    # int(extract_value(args['modelParamsFile'], 'total_simulation_time') / extract_value(args['modelParamsFile'], 'step_size'))
...
def input_pact_parse(args, input_file):
    ...
    grid_height = int(extract_value(args['modelParamsFile'], 'rows'))
    grid_width = int(extract_value(args['modelParamsFile'], 'cols'))
    layers = int(extract_value(args['modelParamsFile'], 'layer'))
...
def load_train_data(args):
    ...
    temp_tensors = []
    count = 0
    num_sim_steps = int(extract_value(args['modelParamsFile'], 'total_simulation_time') / extract_value(args['modelParamsFile'], 'step_size')) + 2
    ...
    for filename in os.listdir(cir_directory):
        if filename.endswith('.cir'):
            ...
            count += 1
        if count == args['n_training_samples']:    #change count to however many .cir/.csv pairs there are in the training folder
            break
...
def train_model(args, power_tensors, temp_tensors):
    ...
    H, W = int(extract_value(args['modelParamsFile'], 'rows')), int(extract_value(args['modelParamsFile'], 'cols'))   # grid dimensions
    ...
    for sample_idx in range(len(power_tensors)):
        ...
        count += 1
        if count == args['n_training_samples']:
            break
    ...
    '''
    # --- Guard: zero-variance columns can create Inf/NaN after scaling ---
    if np.any(~np.isfinite(scaler_X.scale_)) or np.any(scaler_X.scale_ == 0):
        mask = (~np.isfinite(scaler_X.scale_)) | (scaler_X.scale_ == 0)
        scaler_X.scale_[mask] = 1.0
        # re-transform with corrected scales
        X_train_scaled = scaler_X.transform(X_train)

    if np.any(~np.isfinite(scaler_y.scale_)) or np.any(scaler_y.scale_ == 0):
        mask_y = (~np.isfinite(scaler_y.scale_)) | (scaler_y.scale_ == 0)
        scaler_y.scale_[mask_y] = 1.0
        y_train_scaled = scaler_y.transform(y_train)

    # sanity checks
    assert np.isfinite(X_train_scaled).all(), "Non-finite in X_train_scaled"
    assert np.isfinite(y_train_scaled).all(), "Non-finite in y_train_scaled"
    '''
    ...
    '''
        # Guards before solving for beta (closed-form)
    if not np.isfinite(X_train_reduced).all():
        raise ValueError("Non-finite in X_train_reduced")
    if not np.isfinite(y_train_reduced).all():
        raise ValueError("Non-finite in y_train_reduced")
    '''
...
def predict_next_temp(args, power_map, current_temp, scaler_X, pca_features, beta, pca_targets, scaler_y):
    ...
    '''
    # Guards before/after scaling
    if not np.isfinite(feature).all():
        raise ValueError("Non-finite in feature (power||temp)")
    feature = np.nan_to_num(feature, nan=0.0, posinf=0.0, neginf=0.0)

    # Standardize the feature using the previously fitted scaler
    feature_scaled = scaler_X.transform(feature)
    if not np.isfinite(feature_scaled).all():
        raise ValueError("Non-finite after scaler_X.transform")

    # Project the scaled feature using the PCA for features
    feature_reduced = pca_features.transform(feature_scaled)
    if not np.isfinite(feature_reduced).all():
        raise ValueError("Non-finite after pca_features.transform")
    '''
    ...
    '''
    if not np.all(np.isfinite(pred_reduced)):
        raise ValueError("Non-finite in pred_reduced (linear model output)")
    '''
    ...
    if not np.all(np.isfinite(pred_scaled)):
        raise ValueError("Non-finite after pca_targets.inverse_transform")
    '''
...
def predict_temp(args, file_name, scaler_X, pca_features, beta, pca_targets, scaler_y):

    df = input_pact_parse(args, os.path.join(args['test_dir'], file_name))
    power_data = df.values  # shape: (num_timesteps, num_power_nodes)
    ...
    grid_height, grid_width = int(extract_value(args['modelParamsFile'], 'rows')), int(extract_value(args['modelParamsFile'], 'cols'))
    ...
    num_sim_steps = int(extract_value(args['modelParamsFile'], 'total_simulation_time') / extract_value(args['modelParamsFile'], 'step_size')) + 2  # example; adjust as needed
    ...
    # ---------------------------------------------------------------------------------
    # Saving outputs
    save_dir = os.path.join(args['test_dir'], '..', 'MLCAD_predicted_results')
    ...
    if args['visualize']:
        ...
        for i, t in enumerate(time_steps_to_plot):
            ...
            plt.imshow(predicted_seq[t], cmap="hot")
            plt.colorbar()

            # Saving image                
            saved_image_path = os.path.join(save_dir, f"{file_basename}_map-{t}.png")
        ...
        T, X, Y =  predicted_seq.shape[0], predicted_seq.shape[1], predicted_seq.shape[2]
        flat = predicted_seq.reshape(T, X * Y)

        csv_dir = glob.glob(os.path.join(args['train_dir'], "*.cir.csv"))
        ...
        saved_csv_path = os.path.join(save_dir, f'{file_basename}_table.csv')
