#!/usr/bin/env python3
"""Run balanced shards in completed ISA slots and explicitly released NVDLA slots."""
...
def record(event, **details):
    value = {'utc': datetime.now(timezone.utc).isoformat(), 'event': event,
             'baseline_capacity': baseline_capacity, **details}
...
record('waiting_for_capacity', baseline_slots_after_isa=2, explicit_extra_slots=0,
...
while pending or active:
    if baseline_capacity == 0 and (work / 'cpu-isa/summary.json').is_file():
    ...
    for name, (process, log, shard) in list(active.items()):
        ...
        del active[name]
    while pending and len(active) < baseline_capacity + extra:
        shard = pending.pop(0)
        ...
        record('shard_started', campaign=name, pid=process.pid, argv=command,
               active_shards=list(active), capacity=baseline_capacity + extra,
