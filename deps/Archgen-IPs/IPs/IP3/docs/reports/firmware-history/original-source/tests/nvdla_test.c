...
static uint8_t expected[BUFFER_SIZE];
...
static void run_job(unsigned hart, unsigned job) {
    ...
    for (unsigned i = 0; i < BUFFER_SIZE; ++i)
        if (dst[i] != expected[i]) fail("numerical/guard mismatch", hart, job, i, dst[i], expected[i]);
    elem = 0;
    for (unsigned g = 0; g < c->channels / 8u; ++g)
        for (unsigned y = 0; y < c->height; ++y)
            for (unsigned x = 0; x < c->width; ++x)
...
static void run_cdp(unsigned hart, unsigned job) {
    ...
    fence();
    for (unsigned i = 0; i < BUFFER_SIZE; ++i)
        if (dst[i] != expected[i]) fail("CDP numerical/guard mismatch", hart, job, i, dst[i], expected[i]);
    for (unsigned i = 0; i < 256; ++i) {
...
static void run_conv(unsigned hart, unsigned job) {
    ...
    fence();
    for (unsigned i = 0; i < BUFFER_SIZE; ++i)
        if (dst[i] != expected[i]) fail("convolution dot-product/guard mismatch", hart, job, i, dst[i], expected[i]);
    reg_write(GLB_S_INTR_STATUS_0, 0xffffffffu);
...
static void run_pdp(unsigned hart, unsigned job) {
    ...
    fence();
    for (unsigned i = 0; i < BUFFER_SIZE; ++i)
        if (dst[i] != expected[i]) fail("pooling numerical/guard mismatch", hart, job, i, dst[i], expected[i]);
    reg_write(GLB_S_INTR_STATUS_0, 0xffffffffu);
