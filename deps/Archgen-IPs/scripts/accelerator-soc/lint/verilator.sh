...
cd -- "$out"
width_args=()
if [[ -n ${VERILATOR_MAX_NUM_WIDTH:-} ]]; then
  [[ $VERILATOR_MAX_NUM_WIDTH =~ ^[1-9][0-9]*$ ]] || {
    printf 'VERILATOR_MAX_NUM_WIDTH must be a positive integer.\n' >&2
    exit 2
  }
  width_args=(--max-num-width "$VERILATOR_MAX_NUM_WIDTH")
fi
args=(--lint-only --top-module ChipTop -DSYNTHESIS --timescale 1ns/1ps
  -Wall -Wno-fatal "${width_args[@]}" --diagnostics-sarif
  --diagnostics-sarif-output "$out/diagnostics.sarif"
  --Mdir "$out/obj_dir" -y "$rtl" +libext+.sv+.v "$rtl/ChipTop.sv" "$mems" "${extra_sources[@]}")
